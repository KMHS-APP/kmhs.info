...
const AcademicCalendar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function fetchAcademicCalendar() {
      try {
        ...
        const response = await fetch(
          "https://open.neis.go.kr/hub/SchoolSchedule?KEY=4e1af35dc1954eb8aaf573eff8653c43&Type=json&ATPT_OFCDC_SC_CODE=J10&SD_SCHUL_CODE=7531381&AA_FROM_YMD=20240301&AA_TO_YMD=20241231&pSize=365",
        );
        if (!response.ok) {
    ...
    fetchAcademicCalendar().catch((error) =>
  ...
  const tileContent = ({
    ...
    if (view === "month") {
      const event = academicEvents.find(
        (event) =>
          new Date(event.AA_YMD).toISOString().split("T")[0] ===
